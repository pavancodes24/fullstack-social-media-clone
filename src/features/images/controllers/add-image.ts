...
export class Add {
  ...
  public async profileImage(req: Request, res: Response): Promise<void> {
    //here image is base64 encoded string
    const result: UploadApiResponse = (await uploads(req.body.image, `${req.currentUser!.userId}`, true, true)) as UploadApiResponse;
    // with this it will invalidate the previous result, but will still have same public id.
    if (!result?.public_id) {
      throw new BadRequestError('File upload: Error occurred. Try again.');
    }
    const url = `https://res.cloudinary.com/dogpykohr/image/upload/v${result.version}/${result.public_id}`; // public id will be same as currentUser userid
  ...
  private async backgroundUpload(image: string): Promise<IBgUploadResponse> {
    ...
    return { version: version.replace(/v/g, ''), publicId }; //look for any character that matches /v and matches with ''
  }
}
