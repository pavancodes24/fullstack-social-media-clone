...
export class SignUp {
  @joiValidation(signupSchema) //everything should match as per schema
  public async create(req: Request, res: Response): Promise<void> {
    const { username, email, password, avatarColor, avatarImage } = req.body;
    const checkIfUserExist: IAuthDocument = await authService.getUserByUsernameOrEmail(username, email);
    if (checkIfUserExist) {
      // if same is user is trying a signup again
    ...
    // Add to redis cache  -> user data to cache
    const userDataForCache: IUserDocument = SignUp.prototype.userData(authData, userObjectId);
    userDataForCache.profilePicture = `https://res.cloudinary.com/dogpykohr/image/upload/v${result.version}/${userObjectId}`;
  ...
  private signupData(data: ISignUpData): IAuthDocument {
    ...
    return {
      ...
      avatarColor,
      createdAt: new Date()
    } as IAuthDocument; //if fails use unknown
  }
