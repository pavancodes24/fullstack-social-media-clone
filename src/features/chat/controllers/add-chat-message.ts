...
export class Add {
  ...
  public async message(req: Request, res: Response): Promise<void> {
    ...
    let fileUrl = '';
    const messageObjectId: ObjectId = new ObjectId(); //need to save it in the cache so generating it instead of mongodb;
    ...
    if (selectedImage.length) {
      const result: UploadApiResponse = (await uploads(req.body.image, req.currentUser!.userId, true, true)) as UploadApiResponse; //req.body.image or selectedImage?
      if (!result?.public_id) {
        throw new BadRequestError(result.message);
      }
      fileUrl = `https://res.cloudinary.com/dogpykohr/image/upload/v${result.version}/${result.public_id}`;
    }
    const messageData: IMessageData = {
      ...
      deleteForMe: false
    }; //neeed to save it in cache and mongodb
  ...
  private emitSocketIOEvent(data: IMessageData): void {
    socketIOChatObject.emit('message received', data); // this event is used to update the main chat page
    socketIOChatObject.emit('chat list', data); // for chat list event to client
  }
