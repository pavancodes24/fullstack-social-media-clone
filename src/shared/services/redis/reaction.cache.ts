...
export class ReactionCache extends BaseCache {
  ...
  public async removePostReactionFromCache(key: string, username: string, postReactions: IReactions): Promise<void> {
    try {
      ...
      const response: string[] = await this.client.LRANGE(`reactions:${key}`, 0, -1); //get all the data inside the list.
      ...
      multi.LREM(`reactions:${key}`, 1, JSON.stringify(userPreviousReaction)); //once we got the reaction next step is to remove using LREM
      await multi.exec();
