...
export class UserCache extends BaseCache {
  ...
  //excluded key is the key of the loggedin user
  ...
  public async getRandomUsersFromCache(userId: string, excludedUserName: string): Promise<IUserDocument[]> {
    try {
      ...
      for (const key of randomUsers) {
        const followerIndex = indexOf(followers, key); // check if user is a follower of the loggedin user
        if (followerIndex < 0) {
          const userHash: IUserDocument = (await this.client.HGETALL(`users:${key}`)) as unknown as IUserDocument;
          replies.push(userHash);
        }
      }
      const excludedUsernameIndex: number = findIndex(replies, ['username', excludedUserName]); // check if the excluded user is in the replies
