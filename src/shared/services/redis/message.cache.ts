...
export class MessageCache extends BaseCache {
  ...
  public async addChatListToCache(senderId: string, receiverId: string, conversationId: string): Promise<void> {
    try {
      ...
      if (userChatList.length === 0) {
        ...
        if (receiverIndex < 0) {
          // findIndex returns -1 if doesnot exist else greater than 0.
          await this.client.RPUSH(`chatList:${senderId}`, JSON.stringify({ receiverId, conversationId }));
        }
      }
    } catch (error) {
  ...
  public async addChatUsersToCache(value: IChatUsers): Promise<IChatUsers[]> {
    try {
      ...
      const users: IChatUsers[] = await this.getChatUsersList(); //check in the list if both users already exist
  ...
  public async updateChatMessages(senderId: string, receiverId: string): Promise<IMessageData> {
    try {
      ...
      const lastMessage: string = (await this.client.LINDEX(`messages:${parsedReceiver}`, -1)) as string; // -1 is for to get last item in the list
      return Helpers.parseJson(lastMessage) as IMessageData;
    } catch (error) {
      log.error(error);
      throw new ServerError('Server error. Try again.');
    ...
    try {
      ...
      if (parsedMessage) {
        ...
        if (type === 'add') {
          ...
          await this.client.LSET(`messages:${conversationId}`, messageIndex, JSON.stringify(parsedMessage)); // remove reaction
        }
      }
      const updatedMessage: string = (await this.client.LINDEX(`messges:${conversationId}`, messageIndex)) as string;
