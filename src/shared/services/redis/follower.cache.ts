...
export class FollowerCache extends BaseCache {
  ...
  //prop  -> is either blocked or blocked by
  public async updateBlockedUserPropInCache(key: string, prop: string, value: string, type: 'block' | 'unblock'): Promise<void> {
    try {
      if (!this.client.isOpen) {
        await this.client.connect();
      }
      //try with hmget in future  -> hmget will be in the form of the array and below one will be string
      ...
      // const dataToSave: string[] = [`${prop}`, JSON.stringify(blocked)]; // no need to do this . has been changed in the latest version.
      multi.HSET(`users:${key}`, `${prop}`, JSON.stringify(blocked)); // without using multi we can use  -> await this.client.HSET(`users:${key}`, dataToSave); //removing the multi whole code.
      await multi.exec();
    } catch (error) {
      log.error(error);
      throw new ServerError('Server error. Try again.');
